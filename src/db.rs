...
impl Db {
    ...
    pub(crate) fn new() -> Db {
        ...
        //let rt = TOKIO_RT.get_or_try_init(|| Runtime::new()).unwrap();
        //rt.spawn(purge_expired_tasks(shared.clone()));
    ...
    /// Iterate expired records
    pub(crate) fn iter(&self) -> Option<Vec<Bytes>> {
        // Acquire the lock, get the entry and clone the value.
        //
        // Because data is stored using `Bytes`, a clone here is a shallow
        // clone. Data is not copied.
        ...
        while let Some((&(when, id), key)) = state.expirations.iter().next() {
            ...
            // Read entry before remove it - hauld
            let value: Bytes = state.entries.get(key).map(|entry| entry.data.clone()).unwrap();
            results.push(value);
            //let entry = KV { key: key.to_string(), value};
            //let serializedEntry = serde_json::to_string(&entry).unwrap();
...
impl Shared {
    ...
    fn purge_expired_keys(&self) -> Option<Instant> {
        ...
        while let Some((&(when, id), key)) = state.expirations.iter().next() {
            ...
            // Publish the entry expired to system channel - hauld
            let sys_channel = "_expired_"; 
            state
            .pub_sub
            .get(sys_channel)
            // On a successful message send on the broadcast channel, the number
            // of subscribers is returned. An error indicates there are no
            // receivers, in which case, `0` should be returned.
            //.map(|tx| tx.send(Bytes::from(serializedEntry)));
